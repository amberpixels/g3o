...
func walkEast(startXM float64, seconds int) ([]geo.Point, []float64) {
	pts := make([]geo.Point, seconds+1)
...
func TestTrackInAreaCrossing(t *testing.T) {
	...
	st, err := geo.TrackInArea(pts, times, sq, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTrackInAreaEdgeCases(t *testing.T) {
	...
	if _, err := geo.TrackInArea(make([]geo.Point, 3), make([]float64, 2), sq, 0); err == nil {
		t.Error("length mismatch: want error")
	}

	st, err := geo.TrackInArea(nil, nil, sq, 0)
	if err != nil || st != (geo.TrackStats{}) {
		t.Errorf("empty track: got %+v, %v; want zero, nil", st, err)
	}

	st, err = geo.TrackInArea([]geo.Point{offsetM(50, 50)}, []float64{0}, sq, 0)
	if err != nil || st != (geo.TrackStats{}) {
		t.Errorf("single point: got %+v, %v; want zero, nil", st, err)
	}

	// A negative time delta (stream glitch) contributes zero, not negative.
	pts := []geo.Point{offsetM(50, 50), offsetM(55, 50)}
	st, err = geo.TrackInArea(pts, []float64{10, 4}, sq, 0)
	if err != nil {
		t.Fatal(err)
	}
	if st.TimeS != 0 {
...
func TestTrackInAreaBufferWidensInside(t *testing.T) {
	...
	st, err = geo.TrackInArea(pts, times, sq, 5)
	if err != nil {
		t.Fatal(err)
	}
